...
def uap_sgd(model, loader, nb_epoch, eps, beta=12, step_decay=0.8, y_target=None, layer_name=None, uap_init=None):
    '''
    INPUT
    model       model
    loader      dataloader
    ...
    per_image_file = open(path + 'deltas_' + str(args.epsilon) + '.npy', 'wb')
    nb_epoch = 1
...
def apply_attack(model, test_generator):
    nb_epoch = args.epochs
    eps = args.epsilon
    beta = args.beta
    step_decay = args.step_decay
    uap_sgd(model, test_generator, nb_epoch, eps, beta, step_decay)
...
if __name__ == '__main__':
    ...
    apply_attack(model, val_generator)
